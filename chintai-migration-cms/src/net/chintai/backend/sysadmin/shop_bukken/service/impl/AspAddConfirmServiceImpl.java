/*
 * $Id: AspAddConfirmServiceImpl.java 3570 2007-12-14 08:55:47Z t-kojima $
 * ---------------------------------------------------------
 * 更新日      更新者         内容
 * ---------------------------------------------------------
...
/**
 * AspAddConfirmServiceの実装クラス
 * 
 * @author Sim-JongYeon
 * @version $Revision: 3570 $
 * Copyright: (C) CHINTAI Corporation All Right Reserved.
 */
public class AspAddConfirmServiceImpl implements AspAddConfirmService {
    ...
    /*
     * (非 Javadoc)
     * @see net.chintai.backend.sysadmin.shop_bukken.service.AspAddConfirmService#aspAreaEnsenCheck(net.chintai.backend.sysadmin.shop_bukken.service.bean.AspAddConfirmInServiceBean)
     */
    public AspAddConfirmOutServiceBean aspAreaEnsenCheck(AspAddConfirmInServiceBean inServiceBean)
        ...
        // 整列された都道府県、沿線タイプをAspAddConfirmOutServiceBeanに設定
        AspAddConfirmOutServiceBean aspAddConfirmOutServiceBean = new AspAddConfirmOutServiceBean();
        BeanUtils.copyProperties(aspAddConfirmOutServiceBean, inServiceBean);

        // 都道府県を設定。
        List<AspAddConfirmOutServiceBean.PrefListDetailView> prefListDetailViewList =
                new ArrayList<AspAddConfirmOutServiceBean.PrefListDetailView>();
        for (int i = 0; i < aspPrefInfoList.size(); i++) {
            AspPrefInfo aspPref = aspPrefInfoList.get(i);
            AspAddConfirmOutServiceBean.PrefListDetailView prefListDetailView =
                aspAddConfirmOutServiceBean.new PrefListDetailView();
        ...
        aspAddConfirmOutServiceBean.setPrefList(prefListDetailViewList);

        // 沿線タイプを設定。
        List<AspAddConfirmOutServiceBean.EnsenTypeListDetailView> ensenTypeListDetailViewList =
                new ArrayList<AspAddConfirmOutServiceBean.EnsenTypeListDetailView>();
        for (int i = 0; i < aspEnsenTypeInfoList.size(); i++) {
            AspEnsenTypeInfo aspEnsenType = aspEnsenTypeInfoList.get(i);
            AspAddConfirmOutServiceBean.EnsenTypeListDetailView ensenTypeListDetailView =
                aspAddConfirmOutServiceBean.new EnsenTypeListDetailView();
        ...
        aspAddConfirmOutServiceBean.setEnsenTypeList(ensenTypeListDetailViewList);

        return aspAddConfirmOutServiceBean;
    }
}
