/*
 * $Id: ReviewBukkenStatusUpdateCommitAction.java 3570 2007-12-14 08:55:47Z t-kojima $
 * ---------------------------------------------------------
 * 更新日      更新者         内容
 * ---------------------------------------------------------
 ...
 */
public class ReviewBukkenStatusUpdateCommitAction extends BaseActionSupport {
    ...
    protected void doExecute(BusinessContext context) throws Exception {
        ...
        ReviewBukkenStatusUpdateCommitInServiceBean inBean =
                new ReviewBukkenStatusUpdateCommitInServiceBean();
        ...
        if (isTokenValid(context.getRequest())) {
            ...
            ReviewBukkenStatusUpdateCommitService service =
                    (ReviewBukkenStatusUpdateCommitService) ac
                            .getBean("reviewBukkenStatusUpdateCommitService");
            ...
            try {
                // 店舗管理テーブル更新
                service.updateBukkenStatus(inBean);

                // 更新成功ログ、メッセージ設定
                loggingService.write("05002", inBean.getUpdId(), SessionStatus.SUCCESS,
                        ApplicationResources.getProperty("review.kanriCd") + kanriCd, this.getClass().getName());
                context.setMessage("INFO.M.REVIEW.0028", ApplicationResources
                        .getProperty("review.bukkenStatus"));
            } catch (ApplicationException e) {
                // 更新失敗ログ
                loggingService.write("05002", inBean.getUpdId(), SessionStatus.FAILURE,
                        ApplicationResources.getProperty("review.kanriCd") + kanriCd, this.getClass().getName());
//                throw e;
