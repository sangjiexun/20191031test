...
<%@ include file="parts/ShopBasicInfo.jsp" %>
    ...
    <html:form action="/shop_listing/KeiyakuRegisterConfirm" onsubmit="joinDate();">
...
<c:choose>
  ...
  <c:otherwise>
      ...
      <div class="add_form">
        ...
        <div class="form_space">
          <label><input type="radio" name="kihonKeiyakuFlg" value="1" checked="checked">契約開始する</label>
          <label><input type="radio" name="kihonKeiyakuFlg" value="0" disabled="disabled">契約終了する</label>
        </div>
      </div>
      <div class="add_form">
...
<c:choose>
  ...
  <c:otherwise>
        ...
        <chintai:DateTag name="linkStartMonth" type="month" todayFix="false" fixValue="${view.linkStartMonth}" />月
        &nbsp;～&nbsp;
        終了年月
...
function kihonDateControl(){
    var thisForm = document.forms[0];
    if (thisForm.kihonKeiyakuFlg[0].checked && thisForm.keiyakuShubetsu[0].checked) {
        thisForm.keiyakuStartYear.disabled = false;
        thisForm.keiyakuStartMonth.disabled = false;
        thisForm.keiyakuEndYear.disabled = false;
        thisForm.keiyakuEndMonth.disabled = false;
