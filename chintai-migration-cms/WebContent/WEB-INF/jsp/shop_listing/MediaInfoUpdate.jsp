...
<c:set var="title" value="出稿情報（仮保存）編集" />
...
<c:if test="${view.webDisplayCnt >= 1}">
  <p>
    <a href="
      <c:url value="MediaInfoUpdateWebInfoCopy.do">
        <c:param name="shopCd" value="${view.shopCd}" />
        <c:param name="updDt" value="${view.updDt}" />
      </c:url>"  onclick="return confirm('出稿情報（WEB表示中）を本画面にコピーをします。よろしいですか？')" >
      &gt;&gt;現在WEBに表示されている出稿情報を出稿情報（仮保存）へコピーする
    </a>
  </p>
</c:if>

    <html:form action="/shop_listing/MediaInfoUpdateConfirm" onsubmit="setHiddenValue();">
...
<c:choose>
  ...
  <c:otherwise>
        ...
        <div class="form_space">
          エリア
          <chintai:Area name="areaCd" onChange="childControl(this.name, 'ensenTypeCd');putEnsenTypeList();" />&nbsp;&nbsp;
          沿線タイプ
          ...
          沿線
          <select name="ensenCd" style="width: 180px" onChange="putEkiList();" id="ensenCd">
          </select>&nbsp;&nbsp;
          駅
          <select name="ekiCd" style="width: 180px" onChange="setDiabledAddEki();" id="ekiCd">
          </select>&nbsp;&nbsp;
...
<c:choose>
  ...
  <c:otherwise>
        ...
        <div class="form_space">
          ...
          都道府県
          <select name="prefCd" style="width: 110px" onChange="putCityList();" id="prefCd">
          </select>
          市区町村
          <select name="cityCd" style="width: 180px" onChange="setDiabledAddCity();" id="cityCd">
          </select>
...
function setDiabledAddEki(){
  ...
  for(loop = 0; loop < thisForm.selectEki.length; loop++){
    if (thisForm.ekiCd.selectedIndex != -1) {
        ...
        thisForm.selectEki[loop].value == thisForm.ensenCd[thisForm.ensenCd.selectedIndex].value + thisForm.ekiCd[thisForm.ekiCd.selectedIndex].value
    } else {
    	isSelectEki = true;
    }
  }
