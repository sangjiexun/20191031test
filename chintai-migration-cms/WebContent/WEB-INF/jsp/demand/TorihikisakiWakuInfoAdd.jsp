...
<c:set var="title" value="取引先 物件枠設定情報登録" />
<c:set var="view"  value="${TorihikisakiWakuInfoAddView}" />
...
<!--
 ...
 addEvent('load', window, function(){document.getElementsByName('applyYear')[0].focus();});
-->
</script>
...
<%@ include file="../parts/PopUpContentsHeadline.jsp" %>
  ...
  <html:form action="demand/TorihikisakiWakuInfoAddConfirm.do" onsubmit="setSubmit();">
    <div class="add_form">
      ...
      <div>
        <c:choose>
          <c:when test="${empty view.tokusenCnt or view.tokusenCnt eq '0'}">
            <label><input type="radio" name="tokusenCnt" maxlength="5" size="3" value="0"  onclick="chechTokusenCnt(this);" checked>０件</label>
          </c:when>
          <c:otherwise>
            <label><input type="radio" name="tokusenCnt" maxlength="5" size="3" value="0"  onclick="chechTokusenCnt(this);">０件</label>
          </c:otherwise>
        </c:choose>
        <c:choose>
          <c:when test="${view.tokusenCnt eq '10'}">
            <label><input type="radio" name="tokusenCnt" maxlength="5" size="3" value="10" onclick="chechTokusenCnt(this);" checked>１０件</label>
          </c:when>
          <c:otherwise>
            <label><input type="radio" name="tokusenCnt" maxlength="5" size="3" value="10" onclick="chechTokusenCnt(this);">１０件</label>
          </c:otherwise>
        </c:choose>
        <c:choose>
          <c:when test="${view.tokusenCnt eq '20'}">
            <label><input type="radio" name="tokusenCnt" maxlength="5" size="3" value="20" onclick="chechTokusenCnt(this);" checked>２０件</label>
          </c:when>
          <c:otherwise>
            <label><input type="radio" name="tokusenCnt" maxlength="5" size="3" value="20" onclick="chechTokusenCnt(this);">２０件</label>
          </c:otherwise>
        </c:choose>
        <c:choose>
          <c:when test="${view.tokusenCnt eq '50'}">
            <label><input type="radio" name="tokusenCnt" maxlength="5" size="3" value="50" onclick="chechTokusenCnt(this);" checked>５０件</label>
          </c:when>
          <c:otherwise>
            <label><input type="radio" name="tokusenCnt" maxlength="5" size="3" value="50" onclick="chechTokusenCnt(this);">５０件</label>
          </c:otherwise>
        </c:choose>
        <c:choose>
          <c:when test="${view.tokusenCnt eq '999999'}">
            <label><input type="radio" name="tokusenCnt" maxlength="5" size="3" value="999999" onclick="chechTokusenCnt(this);" checked>その他</label>
            <input type="text" name="tokusenCntNote" maxlength="4" size="4" value="${view.tokusenCntNote}">件
          </c:when>
          <c:otherwise>
            <label><input type="radio" name="tokusenCnt" maxlength="5" size="3" value="999999" onclick="chechTokusenCnt(this);">その他</label>
